...
int jelinek(unsigned int *metric,   /* Final path metric (returned value) */
            unsigned int *cycles,   /* Cycle count (returned value) */
            unsigned char *data,    /* Decoded output data */
            unsigned char *symbols, /* Raw deinterleaved input symbols */
            unsigned int nbits,     /* Number of output bits */
            unsigned int stacksize, struct snode *stack,
            ...
            unsigned int maxcycles) /* Decoding timeout in cycles per bit */
{
  ...
  unsigned int stackptr =
      1; // pointer values of 0 are reserved (they mean that a bucket is empty)
  ...
  for (i = 1; i <= ncycles; i++) {
#ifdef DEBUG
    printf("***stackptr=%ld, depth=%d, gamma=%d, encstate=%lx, bucket %d, "
           "low_bucket %d, high_bucket %d\n",
    ...
    if (depth < 56) {
      encstate = encstate << 1;
      ENCODE(lsym,
             encstate); // get channel symbols associated with the 0 branch
    } else {
    ...
    totmet0 =
        gamma + metrics[depth][lsym]; // total metric for 0-branch daughter node
    totmet1 =
        gamma +
        metrics[depth][3 ^ lsym]; // total metric for 1-branch daughter node
    depth++;                      // the depth of the daughter nodes

    bucket = (totmet0 >> 5) +
             200; // fast, but not particularly safe - totmet can be negative
    ...
    if (depth <= nbits_minus_ntail) {
      if (stackptr < stacksize_minus_1) {
        ...
        while (buckets[low_bucket] ==
               0) { // write latest to where the top of the lowest bucket points
          low_bucket++;
        }
        ptr = buckets[low_bucket];
        buckets[low_bucket] =
            stack[ptr].jpointer; // make bucket point to next older entry
      }
